...
from .models import MyNestedModel

class MyConsumer(GenericAsyncAPIConsumer):
    queryset = MyNestedModel.objects.all()
    serializer_class = MyNestedModelSerializer

    @nested_model_observer(MyNestedModel, MyNestedModelSerializer)
    ...
    def my_nested_model_serializer_activity(self, instance: MyNestedModel, action, **kwargs):
